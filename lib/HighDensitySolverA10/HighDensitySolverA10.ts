...
export interface SimpleRouteJsonRectObstacle {
  type: "rect"
  layers?: string[]
...
const OBSTACLE_CONN_ID = -3

export interface HighDensitySolverA10Props {
...
export class HighDensitySolverA10 extends BaseSolver {
  nodeWithPortPoints: NodeWithPortPoints
  cellSizeMm: number
  viaDiameter: number
  MAX_RIPS: number
  ...
  initialPenaltyFn?: HighDensitySolverA10Props["initialPenaltyFn"]
  ...
  private blockedCellsFlat!: Uint8Array // layers * planeSize; 1 = obstacle
  ...
  constructor(props: HighDensitySolverA10Props) {
    super()
    ...
    this.obstacleMargin = props.obstacleMargin ?? 0.15
  ...
  override getConstructorParams(): [HighDensitySolverA10Props] {
    return [
      {
  ...
  override _setup(): void {
    ...
    for (const seg of this.unsolvedSegs) {
      if (
        this.isBlockedCell(seg.startZ, seg.startRow, seg.startCol) ||
        this.isBlockedCell(seg.endZ, seg.endRow, seg.endCol)
      ) {
        this.error = `Port endpoint for ${this.connIdToName[seg.connId]} overlaps an obstacle`
        this.failed = true
        return
      }
    }
  ...
  private prepopulateObstacleCells(): void {
    if (this.obstacles.length === 0) return

    const cellWidth =
      this.cols > 1
        ? this.nodeWithPortPoints.width / (this.cols - 1)
        : this.nodeWithPortPoints.width
    const cellHeight =
      this.rows > 1
        ? this.nodeWithPortPoints.height / (this.rows - 1)
    ...
    for (const obstacle of this.obstacles) {
      ...
      for (let layer = 0; layer < this.layers; layer++) {
        if (!this.obstacleAppliesToLayer(obstacle, layer)) continue
        const zBase = layer * this.planeSize
        for (let row = 0; row < this.rows; row++) {
          for (let col = 0; col < this.cols; col++) {
            const rawX = this.gridOrigin.x + (col + 0.5) * this.cellSizeMm
            const rawY = this.gridOrigin.y + (row + 0.5) * this.cellSizeMm
            const center = applyAffineTransformToPoint(
              this.gridToBoundsTransform,
              {
                x: rawX,
                y: rawY,
              },
            ...
            if (!this.polygonsOverlap(cellCorners, obstacleCorners)) continue
            const flatIdx = zBase + row * this.cols + col
            this.blockedCellsFlat[flatIdx] = 1
            this.usedCellsFlat[flatIdx] = OBSTACLE_CONN_ID
          }
        }
      }
    }
  ...
  private obstacleAppliesToLayer(
    obstacle: SimpleRouteJsonRectObstacle,
    layer: number,
  ): boolean {
    if (!obstacle.layers || obstacle.layers.length === 0) return true

    const z = this.layerToZ.get(layer)
    for (const rawLayerName of obstacle.layers) {
      const layerName = rawLayerName.toLowerCase()
      if (layerName === "top" || layerName === "f.cu") {
        if (layer === 0) return true
        continue
      }
      if (layerName === "bottom" || layerName === "b.cu") {
        if (layer === this.layers - 1) return true
        continue
      }
      if (
        layerName === `layer${layer}` ||
        layerName === `layer-${layer}` ||
        layerName === `layer_${layer}`
      ) {
        return true
      }
      const numericLayer = Number(layerName)
      if (
        Number.isFinite(numericLayer) &&
        (numericLayer === layer || numericLayer === z)
      ) {
        return true
      }
    }
  ...
  private getObstacleCorners(obstacle: SimpleRouteJsonRectObstacle) {
    const halfWidth = obstacle.width / 2 + this.obstacleMargin
    const halfHeight = obstacle.height / 2 + this.obstacleMargin
  ...
  private isBlockedCell(z: number, row: number, col: number): boolean {
    return this.blockedCellsFlat[(z * this.rows + row) * this.cols + col] === 1
  }

  private isBlockedVia(row: number, col: number): boolean {
    ...
    for (let z = 0; z < this.layers; z++) {
      ...
      for (let i = 0; i < offLen; i++) {
        ...
        if (this.blockedCellsFlat[zBase + r * cols + c] === 1) return true
      }
    }
    return false
  }
  ...
  private computeMoveCostAndRips(
    ...
    if (fromZ !== toZ) {
      // Via transition
      if (
        this.isBlockedCell(toZ, toRow, toCol) ||
        this.isBlockedVia(toRow, toCol)
      ) {
        this._moveCost = -1
        this._moveRipped = r
        return
      ...
      // Lateral movement
      if (this.isBlockedCell(toZ, toRow, toCol)) {
        this._moveCost = -1
        this._moveRipped = r
        return
      }
    ...
    for (let ci = 0; ci < cells.length; ci++) {
      ...
      for (let dr = -marginCells; dr <= marginCells; dr++) {
        for (let dc = -marginCells; dc <= marginCells; dc++) {
          ...
          if (this.blockedCellsFlat[flatIdx] === 1) continue
          if (
  ...
  override visualize() {
    ...
    for (const obstacle of this.obstacles) {
      ...
      rects.push({
        ...
        width: obstacle.width + this.obstacleMargin * 2,
        height: obstacle.height + this.obstacleMargin * 2,
        fill: "rgba(255,0,0,0.18)",
        stroke: "rgba(160,0,0,0.65)",
      })
    }
    ...
    return {
      ...
      title: `HighDensityA10 [${this.solvedRoutes?.size ?? 0} solved, ${this.unsolvedSegs?.length ?? 0} remaining]`,
    }
  }
