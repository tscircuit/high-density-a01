...
import { getNativeNodeBoundsError } from "../getNativeNodeBoundsError"
import {
...
export class HighDensitySolverA11 extends HighDensitySolverA01 {
  ...
  private geometryRepairCount = new Map<ConnectionName, number>()
  ...
  override _setup(): void {
    const boundsError = getNativeNodeBoundsError(this.nodeWithPortPoints)
    if (boundsError) {
      this.failed = true
      this.error = `A11 native routing is ineligible: ${boundsError}`
      return
    }
    super._setup()
  }
