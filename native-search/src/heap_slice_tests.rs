...
#[test]
fn ieee_priorities_and_reuse_preserve_every_entry() {
    let positives = [
        0.0,
        f64::from_bits(1),
...
#[test]
fn empty_pop_keeps_original_panic_and_backing_state() {
    ...
    for state in 0..3 {
        if state == 1 {
            for id in 0..9 {
                push(&mut old, &mut heap, f64::NAN, id);
            }
            while !old.entries.is_empty() {
                pop(&mut old, &mut heap);
            }
        } else if state == 2 {
            push(&mut old, &mut heap, -0.0, 2);
            clear(&mut old, &mut heap);
        }
        let expected = catch_unwind(AssertUnwindSafe(|| old.pop())).unwrap_err();
        let actual = catch_unwind(AssertUnwindSafe(|| heap.pop())).unwrap_err();
        assert_eq!(message(expected), message(actual));
        same(&old, &heap);
    }
}
