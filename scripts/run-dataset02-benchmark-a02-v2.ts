...
if (showHelp) {
  console.log(`
Usage: bun run scripts/run-dataset02-benchmark-a02-v2.ts [options]
...
Examples:
  bun run scripts/run-dataset02-benchmark-a02-v2.ts --concurrency=4
  bun run scripts/run-dataset02-benchmark-a02-v2.ts --mode=strict --limit=20
  bun run scripts/run-dataset02-benchmark-a02-v2.ts --profile --limit=20 --concurrency=1
`)
  process.exit(0)
}
...
const workerScriptUrl = new URL(
  "./run-dataset02-benchmark-a02-v2.worker.ts",
  import.meta.url,
)
...
console.log("Dataset02 benchmark for HighDensitySolverA02_V2")
