...
test("A11 preserves distinct physical pairs and both endpoints within one cell", () => {
  ...
  gridPairs.solve()
  expect(gridPairs.getOutput().find((route) => route.connectionName === "b")).toBeUndefined()
  expect(gridPairs.getOutput().find((route) => route.connectionName === "short")?.route).toHaveLength(1)
  ...
  expect(routes.find((route) => route.connectionName === "b")?.route.at(-1)).toMatchObject({ x: 1, y: 0.005, z: 0 })
  expect(routes.find((route) => route.connectionName === "short")?.route).toMatchObject([
    { x: 0, y: 1, z: 0 }, { x: 0.001, y: 1, z: 0 },
  ])
  expect(routes.some((route) => route.connectionName === "alias" || route.connectionName === "zero")).toBe(false)
