...
import { HighDensitySolverA03, HighDensitySolverFailureCache } from "../../lib"
...
test("A03 reuses failed searches only with the same iteration budget", () => {
  ...
  const original = new HighDensitySolverA03(params, cache)
  original.MAX_ITERATIONS = 3
  original.solve()
  expect(original.failed).toBeTrue()

  const repeated = new HighDensitySolverA03(params, cache)
  ...
  const largerBudget = new HighDensitySolverA03(params, cache)
