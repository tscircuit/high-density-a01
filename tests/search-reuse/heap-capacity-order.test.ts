...
test("search heaps preserve priority and sequence order across growth, reuse, and layer solvers", () => {
  for (const [SolverClass, params] of [
    ...
    const solver = new SolverClass({ ...params, nodeWithPortPoints: prevNext })
    solver.setup()
    ...
    for (let id = 0; id < 4096; id++) {
      ...
      heap.push(f, seq, id)
      entries.push({ f, seq: SolverClass === HighDensitySolverA03 ? seq >>> 0 : seq, id })
    }
    entries.sort(compareEntries)
    expect(heap.size).toBe(4096)
    for (const entry of entries.slice(0, 2048)) expect(heap.pop()).toBe(entry.id)
